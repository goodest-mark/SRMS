    ...
    for student in class_students:
        ...
        cur.execute("""
            SELECT r.subject_name,
                   ...
                   r.marks
            FROM results r
            LEFT JOIN subjects s ON LOWER(TRIM(s.subject_name)) = LOWER(TRIM(r.subject_name))
        ...
        for _, short_name, mark in marks_rows:
            grade = get_grade(mark, level=level)
            subjects.append(short_name)
            marks_row.append(str(mark))
        ...
        total_cols = len(results_data[0])

        results_table = Table(
            results_data,
            colWidths=[6.8 * inch / total_cols] * total_cols
        )
        ...
        elements.append(Paragraph("<b>SUBJECT RESULTS</b>", style_subheader))
        ...
        elements.append(compact_footer)
