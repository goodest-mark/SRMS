...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.level_switch.addItem("O Level", "O_LEVEL")
        self.level_switch.addItem("A Level", "A_LEVEL")
        self.level_switch.setMinimumWidth(140)
        self.level_switch.setStyleSheet("""
            QComboBox{
                color:#111827;
                background:#F8FAFC;
        ...
        current_index = self.level_switch.findData(current_level)
        if current_index != -1:
            self.level_switch.setCurrentIndex(current_index)

        self.level_switch.currentIndexChanged.connect(
            lambda index: self.change_level(
                self.level_switch.itemData(index)
            )
        )
    ...
    def update_sidebar_state(self):
        if self.sidebar_collapsed:
            self.sidebar_widget.setFixedWidth(88)
