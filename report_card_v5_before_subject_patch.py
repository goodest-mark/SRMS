...
def generate_report_book(parent, exam_id, class_name, save_path):
    ...
    cur.execute("SELECT school_name, school_address, school_phone, school_email, school_logo, school_stamp FROM school_profile LIMIT 1")
    profile = cur.fetchone()
    school_name = profile[0] if profile else "SCHOOL MANAGEMENT SYSTEM"
    school_addr = profile[1] if profile else "-"
    school_logo = profile[4] if profile else ""
    school_stamp = profile[5] if profile else ""
